...
import { ArrowLeft, Search, Sparkles, Volume2 } from 'lucide-react-native';
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        {result && !isSearching && (
          <>
            ...
            <TouchableOpacity
              style={[styles.speakBtn, { backgroundColor: isSpeaking ? '#6366F1' : '#6366F1' + '15' }]}
              onPress={handleSpeak}
            >
              <Volume2 size={16} color={isSpeaking ? '#FFF' : '#6366F1'} />
              <Text style={[styles.speakBtnText, { color: isSpeaking ? '#FFF' : '#6366F1' }]}>{isSpeaking ? 'Parar' : 'Ouvir Versículos'}</Text>
            </TouchableOpacity>
          </>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  speakBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 14, borderRadius: 14, marginTop: 12 },
  speakBtnText: { fontSize: 15, fontWeight: '700' as const },
});
